...
const getAllItems = async (req, res) => {
  try {
    const items = await Item.find().populate('creator', 'name');
    res.json(items);
  } catch (error) {
...
// Get the seller's story
const getSellerStory = async (req, res) => {
  const { sellerId } = req.params;

  try {
    const seller = await User.findById(sellerId).select('name story image');
...
module.exports = { getAllItems, getItemById, addItemToCart, getCartItems,removeItemFromCart, getSellerStory };
